...
TOKEN=$(echo -n "$USERNAME:$PASSWORD" | base64)
    ...
    response=$(curl -s -k -H "Authorization: Basic $TOKEN" "${HARBOR_SERVER}/api/v2.0/projects/pharmacy_web_frontend/repositories/pharmacy-website/artifacts?with_tag=true")
